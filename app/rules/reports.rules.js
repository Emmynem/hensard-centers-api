...
export const reports_rules = {
	forFindingReportInternal: [
		check('unique_id', "Unique Id is required")
			...
			.custom((unique_id, { req }) => {
				return REPORTS.findOne({
					...
					if (!data) return Promise.reject('Report not found!');
				});
			})
	],
	forFindingReport: [
	...
	forFindingReportAlt: [
		check('report_unique_id', "Report Unique Id is required")
			.exists({ checkNull: true, checkFalsy: true })
			.bail()
			.custom(report_unique_id => {
				return REPORTS.findOne({ where: { unique_id: report_unique_id, status: default_status } }).then(data => {
					if (!data) return Promise.reject('Report not found!');
				});
			})
	],
	forFindingReportAltOptional: [
		check('report_unique_id')
		...
		check('title', "Title is required")
			...
			.custom((title, { req }) => {
				return REPORTS.findOne({
					...
					if (data) return Promise.reject('Report already exists!');
				});
			}),
		check('other')
			.optional({ checkFalsy: false })
